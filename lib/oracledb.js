module.exports = function (RED) {
	...
	node.connectionPoolSize = parseInt(n.connectionpoolsize || 10);
		...
		if (node.connectionPoolSize < 1) {
			node.connectionPoolSize = 1;
		}
		...
		await oracledb.createPool({
			...
			poolMax       : node.connectionPoolSize,
			poolMin       : node.connectionPoolSize,
			//poolPingInterval: 60,	//default
			//poolPingTimeout: 5,	//default
			//poolTimeout: 60,	//default
		});
                await oracledb.getConnection(function (err, connection) {
                    node.connectionInProgress = false;
                    if (err) {
        ...
        node.query = async function (msg, requestingNode, query, values, resultAction, resultSetLimit) {
		...
		node.connection = await oracledb.getConnection();
